...
def main():
    ...
    batch_size_ = 32
    K = 3
    train_data = LoadData(data_path=["/data/PEMS07/PEMS07.csv","/data/PEMS07/PEMS07.npz"],
                          num_nodes=883,
                          divide_rate=[0.8, 0.2],
                          history_length=12,
                          pre_length = 12,
    ...
    test_data = LoadData(data_path=["/data/PEMS07/PEMS07.csv","/data/PEMS07/PEMS07.npz"],
                          num_nodes=883,
                          divide_rate=[0.8, 0.2],
                          history_length=12,
                          pre_length = 12,
    ...
    test2.to_csv("/StarG2N/pems07/PEMS07_result.csv")
    
    Adam_Epoch = 400
    for i in range(5):
        print("------test{}-------".format(i))
        my_net = GraphConv_v6(in_c=10, hid_c1=64, hid_c2=128, K=2, hidden_dim=128, att_hiddem_size=32, final_output_dim=12)
        ...
        torch.save(my_net, "/StarG2N/pems07/PEMS07_{}.pt".format(epoch + 10))
        ...
        test2.to_csv("/StarG2N/pems07/PEMS07_result.csv",mode="a",header=False)
        
        plt.title("train_loss")
        plt.xlabel("epoch")
        plt.ylabel("loss")
        plt.plot(loss_visio)
        plt.savefig("/StarG2N/pems07/PEMS07_test_{}.png".format(i))
        plt.clf()
