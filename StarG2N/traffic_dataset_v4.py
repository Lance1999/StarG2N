...
class LoadData(Dataset):
    def __init__(self, data_path, num_nodes, divide_rate, history_length, pre_length, train_mode):
        ...
        self.graph = combine_adj(get_adjacent_matrix(distance_file=data_path[0], num_nodes=num_nodes),
                                 steps=3)
    ...
    def slice_data(data, history_length, pre_length, index, train_mode):
        ...
        data_x_change = []
        data_x = data_x.transpose([1, 0, 2])
     
        data_x = data_x.squeeze()
       
        for i in range(len(data_x) - 2):
            data_x_temp = np.hstack((data_x[i], data_x[i+1], data_x[i+2]))
            data_x_change.append(data_x_temp)
        data_x =np.array(data_x_change)
...
if __name__ == '__main__':
    train_data = LoadData(data_path=["/data/PEMS04/PEMS04.csv", "/data/PEMS04/PEMS04.npz"], num_nodes=307, divide_rate=[0.8, 0.2],
                          history_length=12, pre_length=12,
                          train_mode="train")
    test_data = LoadData(data_path=["/data/PEMS04/PEMS04.csv", "/data/PEMS04/PEMS04.npz"], num_nodes=307, divide_rate=[0.8, 0.2],
                          history_length=12, pre_length=12,
                          train_mode="test")
