...
class GraphConv_one_cheb(nn.Module):
    def __init__(self, in_c, hid_c1, K, hidden_dim, att_hiddem_size, final_output_dim):
        super(GraphConv_one_cheb, self).__init__()
        
        self.model_name = "2TSGCN-attention-Chebnet-2"
        self.conv1 = ChebConv(in_c=in_c, out_c=hid_c1, K=K)
        self.act = nn.ReLU()

        self._input_dim = hid_c1 
        self._hidden_dim = hidden_dim
    ...
    def forward(self, flow_x, graph_data, B, N):
        ...
        num_nodes = N // 3
        output_2 = self.acc(output_1, num_nodes)
